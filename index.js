...
function queue(h, smh, sub) {
  return new Promise(async (resolve, reject) => {
    ...
    if (db.list[new URL(h).href]) {
      if (new Date() - new Date(db.list[new URL(h).href]) < 1800000)
        reject("That page was crawled in the past 30 minutes.");
      return;
    }
...
app.get("/api/:query/:page*?", async (req, res) => {
  ...
  if (getRandomInt(1, 25) == 20) {
    resultsJson.push({
      ...
      description:
        "<b>Add a page to cheesgle! It won't take a second, I promise.</b>",
    });
  }
